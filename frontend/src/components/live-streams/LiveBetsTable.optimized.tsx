...
const NonceCell = ({ value }: { value: number }) => (
  <Text 
    size="sm" 
    ff="monospace" 
    fw={500}
...
const MultiplierCell = ({ value }: { value: number }) => {
  const color = useMemo(() => {
    return value >= 1000
      ? "red"
      : value >= 100
...
const DistanceCell = ({ value }: { value: number | null }) => {
  const getDistanceColor = useCallback((dist: number) => {
...
const PayoutCell = ({ value }: { value: number }) => {
  return (
    <Text 
      size="sm" 
      ff="monospace"
...
const DifficultyCell = ({ value }: { value: string }) => {
  const color = useMemo(() => {
    return {
      easy: "green",
      medium: "yellow",
...
const TimeCell = ({ value }: { value: string }) => {
  const { formattedTime, color } = useMemo(() => {
    if (!value) {
      return {
        formattedTime: "-",
        color: "hsl(215.4 16.3% 46.9%)"
    ...
    const timeColor = diffMinutes < 1 
      ? "hsl(120 100% 75%)" // Green for very recent
      : diffMinutes < 5 
      ? "hsl(60 100% 75%)" // Yellow for recent
      : diffMinutes < 30 
      ? "hsl(30 100% 75%)" // Orange for somewhat recent
      : "hsl(210 40% 98%)"; // Default white for older
...
const filterFns = {
  greaterThanOrEqualTo: (row: any, id: string, filterValue: any) => {
    const value = row.getValue(id);
    return Number(value) >= Number(filterValue);
  },
};

// Memoized filter select props
  ...
  const columns = useMemo<MRT_ColumnDef<BetRecord>[]>(() => [
    {
      ...
      Cell: ({ cell }) => <NonceCell value={cell.getValue<number>()} />,
    },
    {
      ...
      mantineFilterSelectProps: filterSelectProps,
      Cell: ({ cell }) => <MultiplierCell value={cell.getValue<number>()} />,
    },
    {
      ...
      Cell: ({ cell }) => <DistanceCell value={cell.getValue<number | null>()} />,
    },
    {
      accessorKey: "amount",
      header: "Amount",
      size: 100,
      enableColumnFilter: false,
      Cell: ({ cell }) => <AmountCell value={cell.getValue<number>()} />,
    },
    {
      accessorKey: "payout",
      header: "Payout",
      size: 100,
      enableColumnFilter: false,
      Cell: ({ cell }) => <PayoutCell value={cell.getValue<number>()} />,
    },
    {
      ...
      filterVariant: "select",
      filterFn: "equals",
      Cell: ({ cell }) => <DifficultyCell value={cell.getValue<string>()} />,
    },
    {
      ...
      size: 150,
      enableColumnFilter: false,
      Cell: ({ cell }) => <TimeCell value={cell.getValue<string>()} />,
    },
  ], []);
